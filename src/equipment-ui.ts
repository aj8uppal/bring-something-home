...
import { CLASSES, ENEMIES, MAX_INVENTORY, MAX_VAULT, RARITIES, SHOP } from '../shared/content';
...
export function inventoryPanel(
  ...
  if (panel === 'forge')
    return `${tabs}${currency}<p class="panel-intro">A better blade. A remembered star. A reason to venture out again.</p>${!safe ? '<div class="notice-box">Return to the Hearth to trade, temper, and craft.</div>' : ''}<div class="shop-grid">${SHOP.map((p) => `<div class="shop-item"><span class="shop-icon">${icon(p.icon)}</span><div><h3>${p.name}</h3><p>${p.description}</p></div><button class="small-button" data-game="buy" data-id="${p.id}" ${!safe || c.gold < p.cost ? 'disabled' : ''}>${icon('coin')} ${p.cost}</button></div>`).join('')}</div><h3 class="minor-heading">TEMPER EQUIPMENT</h3><p class="muted">Each tier adds +4 power. Costs gold and 5 persistent embers. Maximum tier 6.</p><div class="temper-list">${Object.entries(
      c.equipment,
    )
      .map(
        ([slot, item]) =>
          `<div>${icon(item?.icon ?? 'spark')}<span>${item ? esc(item.name) : `No ${slot} equipped`}<small>${item ? `Tier ${item.tier} · ${esc(itemSummary(item))}` : ''}</small></span><button class="small-button" data-game="upgrade" data-id="${slot}" ${!safe || !item || item.tier >= 6 || c.gold < item.tier * 50 || profile.embers < 5 ? 'disabled' : ''}>${item ? `${item.tier * 50} gold + 5 embers` : '—'}</button></div>`,
      )
  ...
  const max = panel === 'vault' ? MAX_VAULT : MAX_INVENTORY;
